...
public class NetworkTopoloygApp {
	public static void main(String[] args) throws IOException
	{
		int n,flag = 0;
		Scanner in = new Scanner(System.in);
		String filepath = "src/lib/NetworkTopologyTest.txt";
		Graph<Vertex,Edge> gra = NetworkTopoloygFactory.creatGraph(filepath);
	...
	private static void work(int n,Scanner in,Graph<Vertex,Edge> gra)
	{
		...
		case 1:
			int nn;
			String labell;
			System.out.println("this graph's type is NetworkTopology so the vertexes' type are \nRouter,Server and Computer.\n");
			...
			case 1:
				int flag0;
				Vertex a;
				System.out.println("input the new vertex's type:1.Router,2.Server,3.Computer");
				...
				if(flag0==1)
				{
					...
					argu = makeArgu(argu,in);
					a = RouterVertexFactory.createVertex(labell, "Router", argu);
					gra.addVertex(a);
				...
				else if(flag0==2)
				{
					String[] argu = new String[1];
					argu = makeArgu(argu,in);
					a = ServerVertexFactory.createVertex(labell, "Server", argu);
					gra.addVertex(a);
					System.out.println("successfully added");
				}
				else if(flag0==3)
				{
					String[] argu = new String[1];
					argu = makeArgu(argu,in);
					a = ComputerVertexFactory.createVertex(labell, "Computer", argu);
					gra.addVertex(a);
					System.out.println("successfully added");
				}
				else
		...
		case 2:
			int num;
			System.out.println("this graph's type is NetworkTopology so the edges' type is only NetworkConnection.\n");
			System.out.println("1. add   2. remove   3.change");
			...
			case 1:
				...
				System.out.println("input new edge's weight");
				int ewei = Integer.valueOf(in.nextLine());
				System.out.println("input the first vertex's label");
				String src = in.nextLine();
				Vertex srcV = getVertex(gra.vertices(),src);
				System.out.println("input the second vertex's label");
				...
				else
				{
					...
					if(fa==null)
					{
						Edge ae = NetworkConnectionFactory.createEdge(elabel, "NetworkConnection", vvv, ewei);
						if(ae!=null)
						{
							gra.addEdge(ae);
							System.out.println("successfully added");
							...
							System.out.println("wrong, type mismatch");
					}
					else
					{
	...
	private static String[] makeArgu(String[] argu,Scanner in)
	{
		boolean flag = false;
		System.out.println("input IP address");
		argu[0] = in.nextLine();
		String[] ips = argu[0].split(".");
		int[] vips = new int[ips.length];
		for(int i = 0;i<ips.length;i++)
		{
			vips[i] = Integer.valueOf(ips[i]);
			if(vips[i]<0||vips[i]>255)
				flag = true;
		}
		while(flag)
		{
			System.out.println("at leatst one number beyond the bound [0,255],input the string again.\n");
			flag = false;
			argu[0] = in.nextLine();
			ips = argu[0].split(".");
			for(int i = 0;i<ips.length;i++)
